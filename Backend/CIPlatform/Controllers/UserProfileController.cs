...
namespace CIPlatform.Controllers
{
    public class UserProfileController : Controller
    {
        ...
        public IActionResult AddUserSkill()
        {
            ...
            #region Fill User Skill Drop-Down
            List<SelectListItem> list1 = new List<SelectListItem>();
            ...
            #endregion Fill User Skill Drop-Down

            return PartialView("_AddSkillPartial", model);
        }

        [HttpPost]
        public IActionResult AddUserSkill(AddUserSkillModel model)
        {
            _db.UserSkills.RemoveRange(_db.UserSkills.Where(x => x.UserId == model.UserId));
            _db.SaveChanges();

            #region Add  Skill
            //if (model.userNewSkill != null)
            //{
            //    var temp = model.userNewSkill;
            //    var numbers = temp?.Split(',')?.Select(Int32.Parse)?.ToList();
            //    foreach (var n in numbers)
            //    {
            //        var skill = new UserSkill();
            //        skill.UserId = int.Parse(HttpContext.Session.GetString("UserId"));
            //        skill.SkillId = n;
